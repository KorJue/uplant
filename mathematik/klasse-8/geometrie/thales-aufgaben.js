...
import { circlesAt, pairPoints } from "./tri-construct.js?v=24";
// Die Bausteine der Thaleskreis-Prüfung und die Tangentenaufgabe selbst stehen im Modell-Modul:
// Abschnitt 5 der Seite baut dieselbe Konstruktion als Werkbank, und beide müssen auf dieselbe
// Zeichnung dieselbe Rückmeldung geben.
import {
  beschriftung,
  geradeDurch,
  kreisUm,
...
// Aufgabe 3 — Tangenten von einem äußeren Punkt. Dieselbe Konstruktion wie die Werkbank in
// Abschnitt 5, nur mit fester Vorgabe statt ziehbarer Punkte: Aufgabenstellung, Schritte, Analyse
// und Prüfung kommen darum unverändert aus dem gemeinsamen Modell.
const K3 = (() => {
  const g = tangentenFigur(GC.pt(180, 220), 70, GC.pt(470, 220));
  return {
    id: "k3",
    kurz: "3. Tangenten",
    titel: "Aufgabe 3 — die beiden Tangenten von P an den Kreis",
    stufe: "mittel",
    aufgabe: TANGENTEN_TASK.aufgabe,
    schritte: TANGENTEN_TASK.schritte,
    vorgabe: () => g,
    zeichne: TANGENTEN_TASK.zeichne,
    analyse: TANGENTEN_TASK.analyse,
    pruefe: TANGENTEN_TASK.pruefe,
  };
})();
