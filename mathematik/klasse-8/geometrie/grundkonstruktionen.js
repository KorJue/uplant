...
// Bei Mittelsenkrechte und Winkelhalbierende kommt eine dritte Phase dazu, in der alle drei Linien
...
import { setupCanvasZoom } from "./canvas-zoom.js?v=12";
import { setupDraggableTriangle } from "./triangle-common.js?v=12";
import { setupFreeConstruction } from "./free-ui.js?v=12";
import { TRI_TASKS, TOL_PT } from "./tri-construct.js?v=12";
import { lineThroughBoth, sameRadius, twoArcIntersections } from "./check-helpers.js?v=12";
...
const EXERCISES = {
  mittelsenkrechte: {
    ...
    why:
      "<strong>Warum klappt das?</strong> Beide Kreise haben denselben Radius. Jeder ihrer Schnittpunkte ist deshalb von A <em>und</em> von B genau diesen Radius entfernt. Zwei solche Punkte legen die Gerade fest — und auf ihr liegen genau alle Punkte mit gleichem Abstand zu A und B. Diese <em>Ortslinie</em> geht automatisch durch die Seitenmitte und steht senkrecht auf AB.",
    freeText:
  ...
  winkelhalbierende: {
    ...
    why:
      "<strong>Warum klappt das?</strong> Der erste Bogen schneidet auf beiden Schenkeln <em>gleich weit</em> von S entfernte Punkte ab. Die beiden gleich großen Kreise darum treffen sich deshalb auf der Symmetrieachse des Winkels. Der Abstand eines Punktes von einer Geraden wird über das Lot gemessen — und genau dieser Abstand ist auf der Winkelhalbierenden zu beiden Schenkeln gleich groß.",
    freeText:
  ...
  seitenhalbierende: {
    ...
    why:
      "<strong>Warum klappt das?</strong> Die Mittelsenkrechte wird hier nur als <em>Hilfsmittel</em> benutzt: Sie liefert den Mittelpunkt M der Seite AB. Die Seitenhalbierende selbst ist dann einfach die Strecke von C nach M — sie steht ausdrücklich <em>nicht</em> senkrecht auf AB. Alle drei Seitenhalbierenden treffen sich im Schwerpunkt S, der jede von ihnen im Verhältnis 2:1 teilt (von der Ecke aus gemessen der längere Teil). Auf einer Pappe ausgeschnitten balanciert das Dreieck genau auf S.",
    freeText:
  ...
  hoehe: {
    ...
    why:
      "<strong>Warum klappt das?</strong> C ist von X₁ und X₂ gleich weit entfernt (beide liegen ja auf demselben Kreis um C), und der Kreuzungspunkt Y der beiden gleich großen Bögen ebenfalls. C und Y liegen damit beide auf der Mittelsenkrechten von X₁X₂ — und die steht senkrecht auf AB. Der Abstand eines Punktes von einer Geraden wird immer über dieses Lot gemessen: Die Höhe ist der Abstand von C zur Geraden AB. Bei einem stumpfen Winkel bei A oder B liegt der Fußpunkt außerhalb der Strecke, auf der Verlängerung.",
    freeText:
    ...
    snapPoints(task, tool) {
      return [task.A, task.B, task.C].concat(heightHelperPoints(task, tool));
    },
    markPoints(task, tool) {
...
// Nur Mittelsenkrechte und Winkelhalbierende haben eine Dreiecks-Phase (alle drei Linien + Kreis).
function triTask() {
  return TRI_TASKS[state.exercise] || null;
}

// ---------- Freies Konstruieren (beide Varianten teilen sich ein Werkzeug) ----------
...
const free = setupFreeConstruction({
  ...
  model() {
    if (state.phase === "free-tri") return triTask().analyze(free.tool, triFree.pts);
    const ex = current();
    return {
      snapPoints: ex.snapPoints(state.task, free.tool),
      marks: (ex.markPoints ? ex.markPoints(state.task, free.tool) : []).map((p) => ({ p, done: false })),
      spentCircles: null,
    };
  },
  check() {
    if (state.phase === "free-tri") {
      const t = triTask();
      return t.check(t.analyze(free.tool, triFree.pts));
    }
    return current().check(state.task, free.tool);
  },
  onDraw(layer) {
    const ex = current();
    if (state.phase !== "free-tri" && ex.extraDraw) ex.extraDraw(state.task, free.tool, layer);
  },
  onClear: () => GS.clearEl(layerConstruct),
});

// ---------- Zeichnen der einzelnen Phasen ----------
...
function renderGuided() {
  // Hier darf nichts aus dem freien Konstruieren stehen bleiben, und das Zeichenwerkzeug wird
  // abgeschaltet, damit Klicks auf die Zeichenfläche nichts zeichnen.
  free.reset();
  ...
  els.instructionBox.innerHTML =
    `<p><strong>Schritt ${state.stepIndex + 1} von ${steps.length}:</strong> ${steps[state.stepIndex].text}</p>` +
    (isLast ? `<p>✅ ${current().done}</p><p class="geo-why">${current().why}</p>` : "");
...
// Phase 2: eine einzelne Linie an der vorgegebenen Figur selbst konstruieren.
function renderFreeSetup() {
  showTriVertices(false);
  GS.clearEl(layerTriTriangle);
  GS.clearEl(layerConstruct);
  els.instructionBox.innerHTML = `<p>${current().freeText}</p><p class="geo-why">${current().why}</p>`;
  els.stepsList.innerHTML = `
    ...
    <li>Verklickt? „↩ Rückgängig“ nimmt den letzten Schritt zurück, Esc bricht einen halb gesetzten Kreis ab.</li>
    <li>Mit „Prüfen“ kontrollieren, mit „Tipp“ einen Hinweis bekommen.</li>
  `;
  free.reset();
}

// Phase 3: alle drei Linien samt Kreis am ziehbaren Dreieck.
function enterFreeTri() {
  ...
  els.instructionBox.innerHTML = `<p>${task.intro}</p><p class="geo-why">${task.why}</p>`;
  els.stepsList.innerHTML = task.steps.map((s) => `<li>${s}</li>`).join("");
  free.reset();
}
...
function phaseListFor(exercise) {
  const list = [
    ...
    ["free", TRI_TASKS[exercise] ? "2. Selbst konstruieren" : "2. Selbst konstruieren am Dreieck"],
  ];
  if (TRI_TASKS[exercise]) list.push(["free-tri", "3. Selbst konstruieren am Dreieck"]);
  return list;
}
...
els.exerciseTabs.addEventListener("click", (e) => {
  ...
  // Seitenhalbierende und Höhe haben keine dritte Phase — dann zurück zur Anleitung.
  if (state.phase === "free-tri" && !TRI_TASKS[state.exercise]) state.phase = "guided";
  renderPhaseTabs();
  state.task = current().newTask();
  refreshAll();
});
...
const triFree = setupDraggableTriangle(svg, layerTriVertices, W, H, GC.randomTriangle(W, H), (pts) => {
